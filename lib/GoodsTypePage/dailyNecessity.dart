...
class DailyNecessity extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => new DailyNecessityState();
}

class DailyNecessityState extends State<DailyNecessity> {

  List<Commodity> commodityList = new List();
  List<Commodity> dailyNecessityList = new List();
        ...
        if (response.data['data'] == null) {
          ...
          setState(() {
            ...
            commodityList.forEach((element) {
              if (element.category == "生活用品"){
                dailyNecessityList.add(element);
              }
            });
            print("digital");
            print(dailyNecessityList);
          });
        }
      }
    }
  ...
  _transferIntoLocalList() {
    if(LoginPageState.logged) {
      if (dailyNecessityList.isNotEmpty) {
        reservedList = dailyNecessityList.reversed;//确保时间顺序展示
        print(reservedList);
      ...
      await Future.delayed(Duration(seconds: 1), () {
        ...
        setState(() {
          for (int i = startNum; i <= startNum + 10; i++) {
            startNum = i;
            if (i == dailyNecessityList.length || i == startNum+10) {
              print("没有更多数据了");
              isLoading = false;
              return;
            }
  ...
  Widget build(BuildContext context) {
    return dailyNecessityList.length == 0
        ? noCommodityText()
        : commodityGridView();
  }
