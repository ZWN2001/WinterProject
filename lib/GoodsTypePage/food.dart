...
class Food extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => new FoodState();
}

class FoodState extends State<Food> {

  List<Commodity> commodityList = new List();
  List<Commodity> foodList = new List();
  ...
  _transferIntoLocalList() {
    if(LoginPageState.logged) {
      if (foodList.isNotEmpty) {
        reservedList = foodList.reversed;//确保时间顺序展示
        print(reservedList);
      ...
      await Future.delayed(Duration(seconds: 1), () {
        ...
        setState(() {
          for (int i = startNum; i <= startNum + 10; i++) {
            startNum = i;
            if (i == foodList.length || i == startNum+10) {
              print("没有更多数据了");
              isLoading = false;
              return;
            }
