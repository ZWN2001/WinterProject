...
class UsedBook extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => new UsedBookState();
}

class UsedBookState extends State<UsedBook> {

  List<Commodity> commodityList = new List();
  List<Commodity> usedBookList = new List();
  ...
  _transferIntoLocalList() {
    if(LoginPageState.logged) {
      if (usedBookList.isNotEmpty) {
        reservedList = usedBookList.reversed;//确保时间顺序展示
        print(reservedList);
      ...
      await Future.delayed(Duration(seconds: 1), () {
        ...
        setState(() {
          for (int i = startNum; i <= startNum + 10; i++) {
            startNum = i;
            if (i == usedBookList.length || i == startNum+10) {
              print("没有更多数据了");
              isLoading = false;
              return;
            }
